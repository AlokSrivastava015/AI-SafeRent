...
function OwnerDashboard({ onLogout }) {
  ...
  return <div className="owner-dashboard">
    ...
    <main className="owner-main"><header className="owner-header"><button className="owner-menu" onClick={() => setMenu(!menu)}>☰</button><label className="global-search">⌕<input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search properties, tenants or messages..."/></label><div className="owner-tools"><span className="owner-bell">♧<i /></span><span className="owner-avatar">R</span><button onClick={onLogout}><b>Rohit Sharma</b><small>PG Owner · Sign out</small></button><i>⌄</i></div></header>{active === 'Dashboard' && <><section className="owner-welcome"><div><h1>Welcome back, Rohit!</h1><p>Manage your properties, connect with tenants, and grow your business with AI SafeRent.</p></div><div className="today"><b>▣</b><span><strong>Thursday, 23 Sep 2026</strong><small>Here's what's happening with your properties today.</small></span></div></section>
